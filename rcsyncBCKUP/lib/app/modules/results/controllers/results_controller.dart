  ...
  Future<void> fetchCategoriesAndRanking() async {
    try {
      ...
      if (champResponse.isNotEmpty) {
        ...
        // 1. Buscamos SOLO los IDs en la tabla intermedia (Sin Joins conflictivos)
        final catRelation = await _supabase
            .from('championship_categories')
            .select('id_category')
            .eq('id_championship', champId);

        List<dynamic> catIds = (catRelation as List).map((row) => row['id_category']).toList();

        List<String> fetchedCats = [];

        // 2. Buscamos los nombres por separado
        if (catIds.isNotEmpty) {
          final catsResponse = await _supabase
              .from('categories')
              .select('name')
              .filter('id_category', 'in', catIds);

          fetchedCats = (catsResponse as List)
